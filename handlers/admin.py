...
@admin_router.message(F.text == "Настроить фразы", F.from_user.id.in_(admins))
...
@admin_router.message(Form.phraseNumber, F.text.in_({"1", "2"}), F.from_user.id.in_(admins))
...
@admin_router.message(Form.new_text, F.text, F.from_user.id.in_(admins))
...
@admin_router.callback_query(F.data == "yes", F.from_user.id.in_(admins))
...
@admin_router.callback_query(F.data == "no", F.from_user.id.in_(admins))
async def write_phrase(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_reply_markup(reply_markup=None)
...
@admin_router.message(F.text == 'Выйти из админ панели', F.from_user.id.in_(admins))
